/*
 * Copyright 2020. Heekuck Oh, all rights reserved
 * 이 프로그램은 한양대학교 ERICA 소프트웨어학부 재학생을 위한 교육용으로 제작되었습니다.
 */
#include <stdio.h>
#include <string.h>
#include "aes.h"

static const uint8_t sbox[256] = {
    };

static const uint8_t isbox[256] = {
    };

static const uint8_t Rcon[11] = {0x8d, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36};

static const uint8_t M[16] = {2, 3, 1, 1, 1, 2, 3, 1, 1, 1, 2, 3, 3, 1, 1, 2};

static const uint8_t IM[16] = {0x0e, 0x0b, 0x0d, 0x09, 0x09, 0x0e, 0x0b, 0x0d, 0x0d, 0x09, 0x0e, 0x0b, 0x0b, 0x0d, 0x09, 0x0e};

// type변환 : uint8_t -> uint32_t (메모리 접근 감소)
void u8_to_u32(const uint8_t *a, uint32_t *b)
{
  *b = ((uint32_t)a[3] << 24) | ((uint32_t)a[2] << 16) | ((uint32_t)a[1] << 8) | (uint32_t)a[0];
}

//gf(2^8)에서 a* b mod x^8+x^4+x^3+x+1 계산
uint8_t gf8_mul(uint8_t a, uint8_t b)
{
  uint8_t r = 0;

  while (b > 0)
  {
    if (b & 1)
      r = r ^ a;
    b = b >> 1;
    a = XTIME(a);
  }
  return r;
}

// 왼쪽으로 한 칸 이동
void LRotWord(uint8_t *w)
{
  uint8_t tmp = w[0];
  for (int i = 0; i < 3; i++)
    w[i] = w[i + 1];
  w[3] = tmp;
}

// key expansion에 쓰이는 g function
uint32_t g_func(const uint32_t *roundKey, int roundNum)
{
  uint8_t tmp[4];
  memcpy(tmp, roundKey, 4);

  LRotWord(tmp);
  for (int i = 0; i < 4; i++)
    tmp[i] = sbox[tmp[i]];

  uint32_t res;
  u8_to_u32(tmp, &res);

  return res ^ Rcon[roundNum];
}

/*
 * Generate an AES key schedule
 */
void KeyExpansion(const uint8_t *key, uint32_t *roundKey)
{
  // 사용자가 입력한 key
  for (int i = 0; i < Nk; i++)
    u8_to_u32(key + i * 4, &roundKey[i]);

  // key expansion (각 round에 사용할 roundkey 계산)
  for (int i = Nk; i < RNDKEYSIZE; i++)
  {
    if (i % Nk == 0)
      roundKey[i] = g_func(roundKey + i - 1, i / Nk) ^ roundKey[i - Nk];
    else
      roundKey[i] = roundKey[i - 1] ^ roundKey[i - Nk];
  }
}

static void AddRoundKey(uint8_t *state, const uint32_t *roundKey)
{
  uint8_t *tmp = (uint8_t *)roundKey;
  for (int i = 0; i < BLOCKLEN; i++)
    state[i] ^= tmp[i];
}

static void SubBytes(uint8_t *state, int mode)
{
  if (mode)
  {
    for (int i = 0; i < BLOCKLEN; i++)
      state[i] = sbox[state[i]];
  }
  else
  {
    for (int i = 0; i < BLOCKLEN; i++)
      state[i] = isbox[state[i]];
  }
}

static void ShiftRows(uint8_t *state, int mode)
{
  for (int i = 0; i < 4; i++)
  {
    uint8_t tmp[Nb];
    for (int j = 0; j < Nb; j++)
      *(tmp + j) = *(state + j * 4 + i);

    if (mode)
    {
      for (int j = 0; j < i; j++)
        LRotWord(tmp);
    }
    else
    {
      for (int j = 0; j < Nb - i; j++)
        LRotWord(tmp);
    }

    for (int j = 0; j < Nb; j++)
      state[j * 4 + i] = tmp[j];
  }
}

// gf(2^8)에서의 행렬곱 연산
static void MixColumns(uint8_t *state, int mode)
{

  uint8_t tmp[16], res[4][4];

  if (mode)
    memcpy(tmp, M, sizeof(M));
  else
    memcpy(tmp, IM, sizeof(IM));

  for (int i = 0; i < 4; i++)
  { 
    for (int j = 0; j < 4; j++)
    {
      res[i][j] = 0;
      for (int k = 0; k < 4; k++)
        res[i][j] ^= gf8_mul(tmp[4 * i + k], state[4 * j + k]);
    }
  }
  for (int i = 0; i < 4; i++)
  {
    for (int j = 0; j < 4; j++)
      state[4 * j + i] = res[i][j];
  }
}

/*
 * AES cipher function
 * If mode is nonzero, then do encryption, otherwise do decryption.
 */
void Cipher(uint8_t *state, const uint32_t *roundKey, int mode)
{
  if (mode)
  {
    AddRoundKey(state, roundKey);
    for (int round = 1; round < Nr; round++)
    {
      SubBytes(state, mode);
      ShiftRows(state, mode);
      MixColumns(state, mode);
      AddRoundKey(state, roundKey + Nb * round);
    }
    SubBytes(state, mode);
    ShiftRows(state, mode);
    AddRoundKey(state, roundKey + Nb * Nr);
  }
  else
  {
    AddRoundKey(state, roundKey + Nr * Nb);

    for (int round = Nr - 1; round > 0; round--)
    {
      ShiftRows(state, mode);
      SubBytes(state, mode);
      AddRoundKey(state, roundKey + Nb * round);
      MixColumns(state, mode);
    }
    ShiftRows(state, mode);
    SubBytes(state, mode);
    AddRoundKey(state, roundKey);
  }
}